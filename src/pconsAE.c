...
int main(int argc,char *argv[])             /* Main routine */
{
  ...
  char           temp[PATH_MAX];
  // Parameters for LGsccore-optimizatio
  int            minlength=19; // same as in minatoms in lgscore.c
  double         pval=-9999;
  double         tmppval=-9999;
  double         S=0;
  double         M=20; // for LG_pvalue - default factor
  int            LGstart=0;
  int            LGend=99999;
  int            N=0;
  int            subdom=0;
...
#pragma omp barrier
      ...
      if(files>1)
	{
	  ...
	  for(i=0;i<files;i++)
	    {
	      // here we need to find the optional LGscore for this model
	      pval=9999;
	      LGstart=0;
	      LGend=maxlen;
	      if (subdom)
		{
		  for(j=0;j<maxlen-minlength;j++)
		    {
		      for(k=j+minlength;k<maxlen;k++)
			{
			  N=k-j;
			  S=0.;
			  ...
			  S=M*S/number_of_comparisons[i];
			  tmppval=LG_pvalue(N,S);
			  //if (verbose)
			  //  {
			  //    printf("tmpPval: \"%.3f\" with scores\"\t%f\t%d\t\"in positions \"\t%d\t%d\n",tmppval,S,N,j,k);
			  //  }
			  if (tmppval < pval)
			    {
			      pval=tmppval;
		  ...
		  if (verbose)
		    {
		      printf("Pval: \"%.3f\" in positions \"\t%d\t%d\n",pval,LGstart,LGend);
		    }
		}
	      //LG_average[i]/=number_of_comparisons[i];
	      //S_average[i]/=number_of_comparisons[i];
